...
public class Weapon : WeaponBase
{
    ...
    private float bufferWindow = 0.15f; // 선입력 허용 시간 (초)
    private float bufferExpireTime = 0f;

    public bool isModeChange = false;  // 모드 전환 여부 체크용
    ...
    private Vector2 lastShotRandomPoint;  //  탄퍼짐 랜덤값 저장
    ...
    public override void StartWeaponAction(int type =0)
    {
        ...
        if (type == 0)
        {
            ...
            else
            {
                ...
                {
                    inputBuffered = true;
                    bufferExpireTime = Time.time + bufferWindow;
                }
            }
        }
        // 마우스 오른쪽 클릭 (모드 전환)
    ...
    public override void ThrowWeapon()
    {
        ...
        // 앞으로 날아가는 힘
        rb.AddForce(mainCamera.transform.forward * weaponRT.finalThrowforce, ForceMode.Impulse);
        // 회전 토크 값
    ...
    public void OnAttack()
    {
        ...
        // 뛰고 있을 때는 공격할 수 없다
        if ( animator.MoveSpeed > 0.9f) return;
        ...
        // 비주얼 리코일 적용
        visualRecoil.ApplyVisualRecoil(lastShotRandomPoint, currentSpread, weaponRT.VisualRecoil);
    }
    ...
    private IEnumerator OnModeChange()
    {
        float current = 0;
        float percent = 0;
        float time = weaponRT.finalZoominSpeed;

        animator.PlayAimIn(animator.AimModeIs);
        animator.AimModeIs = !animator.AimModeIs;
        onAimEvent.Invoke(animator.AimModeIs);
        visualRecoil.SetADSMode(animator.AimModeIs);

        float target = animator.AimModeIs ? aimModeFov : defaultModeFOV;

        //  줌 속도를 CameraEffects에 전달
        cameraEffects.SetTargetFOV(target, weaponRT.finalZoominSpeed);
        animator.SetFloat("ZoomSpeed", weaponRT.finalZoominSpeed);

        isModeChange = true;
        while (percent < 1)
        {
            current += Time.deltaTime;
            percent = current / Mathf.Max(time, 0.01f);
            yield return null;
        }

        animator.SetFloat("ZoomSpeed", 1f); // 복구
        isModeChange = false;
    }
    ...
    private void TwoStepRaycast()
    {
        ...
        if ( Physics.Raycast(ray, out hit, weaponRT.finalDistance))
        {
        ...
        {
            targetPoint = ray.origin + ray.direction* weaponRT.finalDistance;
        }
        ...
        if ( Physics.Raycast(bulletSpawnPoint.position, attackDirection, out hit, weaponRT.finalDistance))
        {
            impactMemoryPool.SpawnImpact(hit);

            if (hit.transform.CompareTag("ImpactEnemy"))
                hit.transform.GetComponentInParent<EnemyFSM>()?.TakeDamage(weaponRT.finalDamage);
            else if (hit.transform.CompareTag("InteractionObject"))
                hit.transform.GetComponent<InteractionObject>().TakeDamage(weaponRT.finalDamage);
        }
        Debug.DrawRay(bulletSpawnPoint.position,attackDirection* weaponRT.finalDistance, Color.blue);
    }
    ...
    private Vector3 GetSpreadDirection()
    {
        ...
        if (movement != null)
        {
            ...
            // 다이빙,슬라이딩은 추가 패널티
            if (movement.IsDiving || movement.IsSliding) spread *= 1.2f;
        ...
        lastShotRandomPoint = Random.insideUnitCircle;

        Vector2 random = lastShotRandomPoint * spread;
        Vector3 direction =
    ...
    {
        if (Time.time - lasetAttackTime < weaponRT.finalAttackRate) return false;
        if (animator.MoveSpeed > 0.9f) return false;
        if (weaponSetting.currentAmmo <= 0) return false;
